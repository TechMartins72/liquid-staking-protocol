...
const resolve = async (
  ...
  rli: Interface,
  logger: Logger
): Promise<HydraStakeAPI | null> => {
  let api: HydraStakeAPI | null = null;

  while (true) {
    const choice = await rli.question(DEPLOY_OR_JOIN_QUESTION);
    switch (choice) {
      case "1":
        api = await HydraStakeAPI.deployHydraStakingContract(providers, {
          scaleFactor: 1_000_000,
          validAssetContractAddress: "0200df7d34b0d9843ac09e18d412640a8214836c9dec943e708fde38ee2c3113975f",
          stDomainSeperator: "hydra:htDUST",
          delegationContractAddress: "0200df7d34b0d9843ac09e18d412640a8214836c9dec943e708fde38ee2c3113975f"
        });
        logger.info(
      ...
      case "2":
        api = await HydraStakeAPI.joinHydraStakingContract(
          providers,
...
const displayLedgerState = async (
  providers: HydraStakingContractProviders,
  deployedStateraContract: DeployedHydraStakingOnchainContract,
  logger: Logger
): Promise<void> => {
  ...
  console.log(`Current stakers size is:`, ledgerState.stakings.size());
  console.log(`Current scale factor is:`, ledgerState.SCALE_FACTOR);
  console.log(`Current stablecoin color is:`, ledgerState.stAssetCoinColor);
  console.log(`Current valid stake asset is:`, ledgerState.validAssetCoinType);
  console.log(`Current stake pool is:`, ledgerState.stakePoolStatus == StakePoolStatus.available ? "AVAILABLE" : "DELEGATED");
  }
};

const displayDerivedLedgerState = async (
  currentState: DerivedHydraStakingContractState,
  logger: Logger
): Promise<void> => {
  logger.info(
    `Current admin is: ${currentState.admins}`
  );
  console.log(
    `Current stake pool amount is:`,
    currentState.protocolTVL.value
  );
  console.log(`Current total value minted is:`, currentState.total_stAsset_Minted);
  console.log(`Current stakers size is:`, currentState.stakings.size());
  console.log(`Current scale factor is:`, currentState.scaleFactor);
  console.log(`Current stablecoin color is:`, currentState.stAssetCoinColor);
  console.log(`Current valid stake asset is:`, currentState.validAssetCoinType);
  console.log(`Current stake pool is:`, currentState.stakePoolStatus == StakePoolStatus.available ? "AVAILABLE" : "DELEGATED");
};
...
You can do one of the following:
  1. Set stAsset token color (ADMIN ONLY)
  2. Deposit tDUST into stake pool (STAKERS ONLY)
  3. Redeem tDUST from stake pool (STAKERS ONLY)
  4. Delegate to thrid party contract (ADMIN ONLY)
  5. Recieve delegation reward (THIRD PARTY ONLY)
  6. Display ledger state
  7. Display derived ledger state
  8. Display the current private state (known by you alone)
  9. Display comprehensive wallet state (NEW)
  10. Exit
...
const circuit_main_loop = async (
  ...
  const HydraStakeAPI = await resolve(providers, rli, logger);
  if (HydraStakeAPI === null) return;

  let currentState: DerivedHydraStakingContractState | undefined;
  const stateObserver = {
    next: (state: DerivedHydraStakingContractState) => {
      currentState = state;
    },
  };

  const subscription = HydraStakeAPI.state.subscribe(stateObserver);
  ...
  try {
    while (true) {
      ...
      switch (choice) {
        case "1": {
          await HydraStakeAPI.setTokenType();
          break;
        }

        case "2": {
          await HydraStakeAPI.stake(
            Number(await rli.question("Enter amount you want to stake: "))
          );
          break;
        }

        case "3": {
          await HydraStakeAPI.redeem(
            Number(await rli.question("Enter amount you want to payback to redeem your asset: "))
          );
          break;
        }

        case "4": {
          await HydraStakeAPI.delegate(
          );
          break;
        }

        case "5": {
          await HydraStakeAPI.recieveDelegateRewards(
            Number(await rli.question("Enter amount of reward to deposit in the contract: "))
          );
          break;
        }
        ...
        case "6": {
          await displayLedgerState(
            providers,
            HydraStakeAPI.allReadyDeployedContract,
            logger
          );
          break;
        }
        case "7": {
          await displayDerivedLedgerState(
            currentState as DerivedHydraStakingContractState,
            logger
          );
          break;
        }
...
const displayComprehensiveWalletState = async (
  wallet: Wallet,
  currentContractState: DerivedHydraStakingContractState | undefined,
  logger: Logger
): Promise<void> => {
  const state = await Rx.firstValueFrom(wallet.state());
  ...
  Object.entries(state.balances).forEach(([tokenType, balance]) => {
    if (tokenType === nativeToken()) {
      ...
      tokenType === currentContractState.stAssetCoinColor
    ) {
